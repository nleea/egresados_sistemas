...
class FacultiesSerializers(BaseSerializers):
    ...
    def update(self, instance, validated_data):
        instance.name = validated_data.get("name", instance.name)
        instance.headquarter_id = validated_data.get("sede", instance.headquarter)
        instance.save()
        return instance
...
class ProgramsSerializersView(BaseSerializers):
    ...
    def to_representation(self, instance):
        return {
            "id": instance.id,
            "name": instance.name,
            "faculty": {"id": instance.faculty.id, "name": instance.faculty.name},
        }
...
class ProgramsSerializers(BaseSerializers):
    ...
    def update(self, instance, validated_data):
        instance.name = validated_data.get("name", instance.name)
        instance.faculty_id = validated_data.get("faculta", instance.faculty)
        instance.save()
        return instance
