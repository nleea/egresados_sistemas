...
class PqrsView(APIView):
    def get_pqrs_date(self, status, order, start, end):
        pqrs_filter = (
            Pqrs.objects.defer(
                "tipopqrs__userCreate_id", "tipopqrs__userUpdate_id", "userUpdate"
            )
            .select_related("persona", "tipopqrs", "userCreate")
            .prefetch_related(
                models.Prefetch(
                    "asignacion_set",
            ...
            .filter(
                userCreate=self.request.user.pk,
                status=status,
    ...
    def get(self, request, *args, **kwargs):
        ...
        if roles:
            pqrs_filter = (
                ...
                .select_related("persona", "tipopqrs")
                .prefetch_related(
                    models.Prefetch(
                        "asignacion_set",
                        queryset=Asignacion.objects.all().defer(
                ...
                .filter(visible=True, status=status_pqrs)
                .order_by(order_pqrs)
            )
            ...
            pqrs_filter = (
                ...
                .filter(userCreate=request.user.id, visible=True)
                .order_by(order_pqrs)
            )
            data = PqrsSerializersView(pqrs_filter, many=True, meta=False)
            return Response(data.data, status.HTTP_200_OK)
...
class DeletePqrsView(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if instanceOrNone is None:
            return Response(
                "Pqrs {} not exist".format(self.kwargs.get("pk")),
