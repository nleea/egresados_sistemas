...
class ClassifiedController(BaseController):
    ...
    def state_changes(self, id, data):
        instance, status = self.get_object(id=id, instance=True)
        ...
        if not state and mensajes:
            ...
            return m_instance.errors, status
        return "Ok", status
    ...
    def get_object(self, id, instance=False, prefetch=None, related=None):
        if instance:
            exist = self.repo.get_instance(id, prefetch, related)

            if exist:
                return exist, status.HTTP_200_OK
            return "NOT FOUND", status.HTTP_400_BAD_REQUEST

        serializer = self.serializer(
            [self.repo.get_instance(id, prefetch, related)], many=True
        )
